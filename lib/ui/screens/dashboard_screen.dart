...
class DashboardScreen extends StatelessWidget {
  ...
  void _onTake(BuildContext context, DoseConfirmation dose) async {
    ...
    await context.read<AdherenceProvider>().confirmDoseTaken(dose.id, auth.patient!.id);
    if (context.mounted) {
          ...
          SliverAppBar(
            ...
            flexibleSpace: FlexibleSpaceBar(
              ...
              title: Column(mainAxisAlignment: MainAxisAlignment.end, crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(isAr ? 'مرحباً، ${auth.patient?.name ?? ""}' : 'Hello, ${auth.patient?.name ?? ""} 👋',
                  style: const TextStyle(color: AppColors.white, fontSize: 16, fontWeight: FontWeight.w600)),
                Text(isAr ? '${adh.pendingCount} جرعات اليوم' : '${adh.pendingCount} doses pending today',
          ...
          SliverPadding(
            ...
            sliver: SliverList(delegate: SliverChildListDelegate([
              ...
              GridView.count(
                ...
                children: [
                  MetricTile(label: isAr ? 'الالتزام هذا الشهر' : 'This month', value: '${(rate * 100).round()}%', valueColor: AppColors.teal),
                  MetricTile(label: isAr ? 'مأخوذة اليوم' : 'Taken today', value: '$taken', valueColor: AppColors.green),
                  MetricTile(label: isAr ? 'فائتة اليوم' : 'Missed today', value: '$missed', valueColor: AppColors.amber),
                  MetricTile(label: isAr ? 'معلقة' : 'Pending', value: '${adh.pendingCount}', valueColor: AppColors.grayDark),
                ],
              ),
              ...
              AppCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  SectionLabel(isAr ? 'الالتزام الشهري' : 'Monthly adherence'),
                  AppBadge.adherence(rate >= 0.8 ? (isAr ? 'جيد' : 'Good') : rate >= 0.6 ? (isAr ? 'مقبول' : 'Fair') : (isAr ? 'ضعيف' : 'Poor')),
                ]),
                AdherenceBar(rate: rate),
                const SizedBox(height: 6),
                Text('${(rate * 100).round()}% — $taken taken, $missed missed this month', style: AppTextStyles.medDetail),
              ])),
              const SizedBox(height: AppSpacing.lg),

              // Today header
              Text(isAr ? "أدوية اليوم" : "Today's medications", style: AppTextStyles.screenTitle.copyWith(fontSize: 16)),
              ...
              if (adh.isLoading)
                ...
                EmptyState(
                  ...
                  subtitle: isAr ? 'اضغط + لإضافة أول دواء' : 'Tap + on the Medications tab to add your first medication',
                )
              else if (today.isEmpty)
                AppCard(child: Center(child: Padding(padding: const EdgeInsets.all(24),
                  child: Column(children: [
                    const Icon(Icons.check_circle_outline_rounded, size: 40, color: AppColors.teal),
                    const SizedBox(height: 8),
                    Text(isAr ? 'لا جرعات اليوم 🎉' : 'No doses scheduled today 🎉', style: AppTextStyles.medName),
                  ]))))
              else
...
class _DoseCard extends StatelessWidget {
  ...
  const _DoseCard({required this.dose, required this.onTake, required this.onMiss});
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (dose.confirmedAt != null)
            Text(
              ...
              style: AppTextStyles.medDetail.copyWith(color: dose.isTaken ? AppColors.teal : AppColors.red),
            ),
        ])),
        DoseActionButton(state: btnState, onTake: dose.isPending ? onTake : null, onMiss: dose.isPending ? onMiss : null),
      ]),
    );
  }
}